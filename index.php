...
# Database functions
	...
	function makeEventQuery($eventcdptrn) {
		return "SELECT DATE_FORMAT(e.eventdt, '%a %b %e, %Y') edt, 
					...
					e.mbrprice, e.guestprice, e.eventcd
				FROM `event` e 
	...
	function printOpenCurling() {
		...
		if (getEventTypeCount($eventcdptrn) > 0) {
			...
			while ($row = mysql_fetch_object($res)) {
				$url = "eventstatus.php?eventMask=$row->eventcd";
				if ($ix == 0) {
					$b->header = "Open Curling on $row->edt";

					$b->rightRaw .= "<p>";
					$b->rightRaw .= "The curling season continues in Atlanta! ";
					$b->rightRaw .= "Our next Open Curling event is scheduled for <strong><a href='$url'>$row->edt at $row->etm</a></strong>. ";
					$b->rightRaw .= "</p>";
					...
					$b->bottomRaw .= "<li><a href='$url'>$row->edt at $row->etm</a></li>\n";
				}
		...
		if (getEventTypeCount($eventcdptrn) > 0) {
			...
			while ($row = mysql_fetch_object($res)) {
				...
				if ($ix == 0) {
					...
					$b->rightRaw .= "Our next Learn to Curl is scheduled for <strong><a href='$url'>$row->edt at $row->etm</a></strong>. ";
					$b->rightRaw .= "</p>";
					...
					$b->bottomRaw .= "<li><a href='$url'>$row->edt</a></li>\n";
				}
		...
		if (getEventTypeCount($eventcdptrn) > 0) {
			...
			while ($row = mysql_fetch_object($res)) {
				...
				if ($ix == 0) {
					...
					$b->rightRaw .= "<p>";
					$b->rightRaw .= "Our next Pickup Curling is at <strong><a href='$url'>$row->edt at $row->etm</a></strong>. ";
					$b->rightRaw .= "Come curl with whoever else signs up. Or bring a friend who knows how to curl. Teams will be formed at the event.";
					$b->rightRaw .= "</p>";
